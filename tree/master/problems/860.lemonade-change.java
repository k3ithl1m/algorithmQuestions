...
class Solution {
	...
	for (int i = 0; i < bills.length; i++) {
		...
		int neededChange = bills[i] - 5;
		while (neededChange > 0) {
			Integer higherKey = map.ceilingKey(neededChange);
			if (higherKey != null) {
				Integer getHigherKey = map.get(higherKey);
				if (getHigherKey > 1) map.put(higherKey, map.get(higherKey) - 1);
				else map.remove(higherKey);
				neededChange -= higherKey;
			} else return false;
		}

		map.put(currentBill, map.getOrDefault(currentBill, 0) + 1);
	}
	return gotChange;
    }
}
