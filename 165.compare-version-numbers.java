...
class Solution {
	...
	int version1Pointer = 0, version2Pointer=0;
	
	while (version1Pointer < version1Split.length && version2Pointer < version2Split.length) {
		int version1Val = Integer.parseInt(version1Split[version1Pointer++]);
		int version2Val = Integer.parseInt(version2Split[version2Pointer++]);
		if (version1Val > version2Val) return 1;
		else if (version2Val > version1Val) return -1;
	}

	int tempVal = 0;
	while (version1Pointer < version1Split.length) {
		int version1Val = Integer.parseInt(version1Split[version1Pointer]);
		if (version1Val > tempVal) return 1;
		version1Pointer++;
	}

	while (version2Pointer < version2Split.length) {
		int version2Val = Integer.parseInt(version2Split[version2Pointer++]);
		if (version2Val > tempVal) return -1;
	}
	return 0;
    }
}
